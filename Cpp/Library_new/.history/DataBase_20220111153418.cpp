...
{
    if (book[b.getid()].num== 0)
    {
        book[b.getid()]= b;
    }
    else
    {
        book[b.getid()].setNum(book[b.getid()].num + b.num);
    }
    return 1;
}
...
void DataBase::Savesort()
{
    ...
    DataBase(this->book,this->size).show();
}
...
void DataBase::show(){
    for(int i=1;i<size;i++){
        if(book[i].num!=0){
            ...
            cout << "  " << "价格为:" << book[i].getPrice() << "  " << "数量为:" << book[i].num << "  " << "购买金额为:" << book[i].num*book[i].getPrice() << "  " << "加入购物车日期为:";
            book[i].getSave().show();
            cout << endl;
        }
    }
